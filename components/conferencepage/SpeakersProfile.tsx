...
const SpeakersProfile: FC<CProps> = ({ speakers }) => {
  return speakers?.length ? (
    speakers?.map((speaker: InSpeaker, i: number) => (
      <div
        className="bg-fifth rounded-md p-4 flex gap-7 mobile:gap-3 items-center mobile:items-start"
        key={i}
      >
        <Image
          src={speaker?.image?.url || "/Speaker.png"}
          alt=""
          width={140}
          height={140}
        ...
        />
        <div>
          <div className="flex justify-between mobile:flex-col mobile:justify-normal">
            <Typography.H3 bold>
              <span className="mobile:text-base">{speaker?.name}</span>
            </Typography.H3>
            <div className="flex gap-4 mobile:gap-6 justify-start items-center">
              <Link
                href={speaker?.social?.twitter || "https://twitter.com/"}
                target="_blank"
              >
                <svg
              ...
              <Link
                href={speaker?.social?.linkedin || "https://linkedin.com/"}
                target="_blank"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
              ...
              <Link href={speaker?.social?.homepage || "/"} target="_blank">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="18"
                  height="18"
              ...
              <Link
                href={speaker?.social?.github || "https://github.com/"}
                target="_blank"
              >
                <svg
                  width="18"
    ...
    <div className="bg-fifth rounded-md p-4">
      <Typography.H3 bold>
        <span className="text-seventh">No Speaker Available</span>
      </Typography.H3>
    </div>
  );
};
